...
def age(file, date_column, birthday_column):
    ...
    bdates = [int(bdate[6:]) if bdate != 'NA' else yearNow for bdate in df[birthday_column]]

    dates = [int(date[-2:])  if date != 'NA' else yearNow for date in df[date_column]]

    ages = [(dates[index] - bdate) * -1 for index, bdate in enumerate(bdates)]
